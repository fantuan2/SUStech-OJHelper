...
默认支持 **SUSTech OJ**（`https://acm.sustech.edu.cn`），其他 Hydro 站点可在明确风险提示后按站点启用（Beta）。
...
### 代码规范化
- 在题目提交页 / scratchpad 编辑器中注入「规范化」按钮，一键整理代码。
- **Java**
  - 将包含 `main` 方法的主类重命名为 `Main`（类名与构造器同步替换）。
  - 统一 `main` 方法签名。
  - 删除 `package` 声明、移除 BOM。
  - 可选：去除 `//` 与 `/* */` 注释（保留字符串字面量中的 `//`）。
- **C/C++**：已注册占位，暂未实现规范化。
- 代码解析会跳过字符串与注释，避免误改。
...
已启用站点可在扩展设置页（`options`）的「已启用的 OJ 站点」区块中查看或移除。SUSTech 为默认启用，不可移除。
...
非 SUSTech 站点的支持为 Beta，未经过充分测试，可能与站点已有功能冲突，改写后的代码未必符合该站点的提交要求。使用本扩展所造成的一切后果由使用者自行承担。
